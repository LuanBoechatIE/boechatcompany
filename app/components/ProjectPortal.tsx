...
export type Project = {
  ...
  image?: string;
  /** print de página inteira: poster enquanto o iframe carrega + fallback */
  previewImage?: string;
  /** URL carregada no iframe (preview interativo real) */
  live?: string;
};
...
export function ProjectPortal({
  ...
  const targetWidth =
    device === "desktop"
      ? areaWidth || undefined
      : Math.min(DEVICE_WIDTH[device], areaWidth || DEVICE_WIDTH[device]);
      ...
      >
        ...
        >
          ...
          >
            ...
            {/* Toggle de dispositivo (só quando há preview vivo) */}
            {hasLive && (
          ...
          >
            ...
            >
              {/* Poster (print) enquanto o iframe carrega — evita flash em branco */}
              {project.previewImage && (
                <motion.img
                  src={project.previewImage}
                  alt={`Preview do site ${project.name}`}
                  animate={{ opacity: hasLive && loaded ? 0 : 1 }}
                  transition={{ duration: 0.5 }}
                  className={`absolute inset-0 h-full w-full ${
                    hasLive ? "object-cover object-top" : "object-contain object-top"
                  }`}
                />
              )}
              ...
              {hasLive ? (
                <iframe
                  key={project.live}
                  src={project.live}
                  title={`Preview interativo — ${project.name}`}
                  onLoad={() => setLoaded(true)}
                  loading="lazy"
                  sandbox="allow-scripts allow-same-origin allow-forms allow-presentation"
                  allow="autoplay; fullscreen; encrypted-media; picture-in-picture"
                  referrerPolicy="no-referrer"
                  className="relative h-full w-full border-0 bg-ink"
                />
              ) : (
                project.previewImage && (
              ...
              {/* Spinner enquanto carrega o site vivo */}
              {hasLive && !loaded && (
                <div className="pointer-events-none absolute inset-0 flex items-center justify-center">
                  <span className="flex items-center gap-2 rounded-full border border-ink-line bg-ink/80 px-4 py-2 text-xs text-gelo-dim backdrop-blur">
                    <Loader2 className="h-3.5 w-3.5 animate-spin text-roxo-light" />
                    Carregando o site…
                  </span>
                </div>
              )}
