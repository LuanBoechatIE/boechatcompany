...
export function AdminContas({ perfil }: { perfil: PerfilView }) {
  const podeCriar = perfil.superAdmin || perfil.permissoes.includes("administracao_contas.criar_conta");
  const podeEditar = perfil.superAdmin || perfil.permissoes.includes("administracao_contas.editar_conta");
  const podeExcluir = perfil.superAdmin || perfil.permissoes.includes("administracao_contas.excluir_conta");
  ...
  const [painel, setPainel] = useState<UsuarioAdmin | null>(null);
  ...
  // Mantém o painel aberto com os dados frescos depois de qualquer ação
  // (ex.: editar nome não deve deixar o painel mostrando o valor antigo).
  useEffect(() => {
    if (!painel) return;
    const atualizado = usuarios.find((u) => u.id === painel.id);
    if (atualizado && atualizado !== painel) setPainel(atualizado);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [usuarios]);
  ...
  return (
    <div className="flex flex-col gap-4">
        ...
        <ul className="flex flex-col divide-y divide-ink-line/60">
          {filtrados.map((u) => (
            <li
              key={u.id}
              onClick={() => setPainel(u)}
              className="flex cursor-pointer flex-col gap-3 py-3 first:pt-0 last:pb-0 hover:bg-ink/40 sm:flex-row sm:items-center"
            >
              ...
              <div className="flex flex-wrap items-center gap-1.5" onClick={(e) => e.stopPropagation()}>
                {u.excluido ? (
                  podeExcluir && <button onClick={() => { if (window.confirm(`Restaurar a conta de ${u.nome}?`)) acao(() => { const fd = new FormData(); fd.set("id", String(u.id)); return restaurarUsuario(fd); }, "Conta restaurada."); }} className="flex items-center gap-1.5 rounded-lg border border-emerald-500/30 bg-ink px-2.5 py-1.5 text-xs text-emerald-300 hover:bg-emerald-500/10"><RotateCcw className="h-3.5 w-3.5" /> Restaurar</button>
                ) : (
                  <>
                    {u.protegido ? (
                      ...
                      <>
                        ...
                        {podeExcluir && <button onClick={() => setExcluir(u)} className="flex items-center gap-1.5 rounded-lg border border-red-500/30 bg-ink px-2.5 py-1.5 text-xs text-red-300/80 hover:bg-red-500/10"><Trash2 className="h-3.5 w-3.5" /> Excluir</button>}
                      </>
                    )}
                  </>
                )}
      ...
      {novo && podeCriar && <NovoUsuarioModal cargos={cargos} onClose={() => setNovo(false)} onSalvar={(fd) => acao(() => criarUsuario(fd), "Usuário criado.", () => setNovo(false))} />}
      {painel && (
        <FuncionarioPainel
          usuario={painel}
          cargos={cargos}
